module BxBlockShoppingCart
  class OrdersController < ApplicationController
    before_action :get_user, only: %i(show create)
    before_action :find_order, only: %i(show)

    def index
      @orders = if params[:filter_by].present? && params[:filter_by].downcase == 'history'
        BxBlockShoppingCart::Order.where(status: ['completed', 'cancelled'])
      elsif params[:filter_by].present?
        BxBlockShoppingCart::Order.where(status: params[:filter_by])
      end
